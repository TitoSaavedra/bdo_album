...
<div class="controls">
  ...
  <div class="ctrl-section">
    <span class="ctrl-label">{$_('scrapper.title')}</span>
    <span class="status-badge status-{status}">{$_(`scrapper.status.${status}`)}</span>
  </div>
  ...
  {#if status === 'running'}
    ...
    <!-- Progress -->
    <div class="ctrl-section">
      <span class="ctrl-label">{$_('scrapper.progress.label')}</span>
  ...
  <!-- CF Token -->
  <div class="ctrl-section">
    <span class="ctrl-label">{$_('scrapper.token.label')}</span>
    <Input
      bind:value={cfToken}
      type="password"
      placeholder={$_('scrapper.token.placeholder')}
      disabled={isBusy}
    />
  </div>
  ...
  {#if status === 'running'}
    <Button variant="ghost" onclick={stop}>
      {$_('scrapper.actions.stop')}
    </Button>
  {:else if status === 'stopping'}
    <Button variant="ghost" disabled>
      {$_('scrapper.actions.stopping')}
    </Button>
  {:else}
    <Button variant="primary" onclick={start} disabled={!cfToken}>
      {$_('scrapper.actions.start')}
    </Button>
  {/if}
