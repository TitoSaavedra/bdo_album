...
impl PresetRepository {
    ...
    pub async fn get_all_ids(pool: &PgPool) -> Result<std::collections::HashSet<i64>> {
        let ids: Vec<i64> = sqlx::query_scalar("SELECT id FROM scraper_presets")
            .fetch_all(pool)
            .await?;
    ...
    pub async fn get_stats(pool: &PgPool) -> Result<serde_json::Value> {
        let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM scraper_presets")
            .fetch_one(pool).await?;

        let with_images: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM scraper_presets WHERE image_1_url IS NOT NULL",
        ).fetch_one(pool).await?;
