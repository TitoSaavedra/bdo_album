...
impl PendingModificationRepository {
    /// Stages raw pasted image bytes — album never talks to R2 directly for
    /// this feature; the scraper's `preset_modifications` worker drains this
    /// queue every 60s, uploads to R2, and purges the row (see the migration
    /// and `scraper/core/src/scraper/preset_modifications.rs`).
    pub async fn insert(
        pool:          &PgPool,
        preset_id:     i64,
        image_1_bytes: &[u8],
        ...
        sqlx::query(
            r#"
            INSERT INTO album_pending_preset_modifications (preset_id, image_1_data, image_2_data)
            VALUES ($1, $2, $3)
            "#,
        )
        .bind(preset_id)
