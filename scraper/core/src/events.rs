...
// ── Sink ─────────────────────────────────────────────────────
// Replaces the `&AppHandle` that used to be threaded through the scraper
// engine purely to emit live Tauri events (`Events::*`, best-effort). The GUI
// implements this with `TauriSink` (src-tauri/src/tauri_sink.rs), forwarding
// each call to `app.emit(...)`. The headless CLI implements it with `CliSink`
// (cli/src/cli_sink.rs), forwarding `log_entry` to `tracing` and no-op'ing the
// rest — everything that matters is already persisted to
// `scraper_sessions`/`scraper_logs`/`scraper_class_stats` regardless of sink.
//
// One method per event, named after the event string the frontend listens to.
// Adding a new event = add a payload struct above + a method here + implement
// it in both TauriSink and CliSink.
